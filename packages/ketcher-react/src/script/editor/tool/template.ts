...
class TemplateTool implements Tool {
  ...
  private sgroupIdsBefore: Array<number> = [];
  ...
  async mousedown(event: MouseEvent) {
    ...
    this.sgroupIdsBefore = Array.from(this.editor.struct().sgroups.keys());
  ...
  mouseup(event?: Event) {
    ...
    this.removeNewFunctionalGroupAbbreviations(restruct);
    if (mouseEvent) {
      this.editor.hover(
  ...
  removeNewFunctionalGroupAbbreviations(restruct): void {
    const molecule = restruct.molecule;
    const before = new Set(this.sgroupIdsBefore);
    const newSgroupIds = Array.from(molecule.sgroups.keys()).filter(
      (id) => !before.has(id as number),
    );
    const removeAction = new Action();
    newSgroupIds.forEach((id) => {
      const sgroup = molecule.sgroups.get(id);
      const fg = sgroup
        ? FunctionalGroup.findFunctionalGroupBySGroup(
            molecule.functionalGroups,
            sgroup,
          )
        : undefined;
      const isSaltOrSolvent = sgroup
        ? SGroup.isSaltOrSolvent(sgroup.name)
        : false;
      if (fg || isSaltOrSolvent) {
        removeAction.mergeWith(fromSgroupDeletion(restruct, id));
      }
    });
    if (!removeAction.isDummy(restruct)) {
      this.editor.update(removeAction);
    }
    this.sgroupIdsBefore = [];
  }
...
function addOnCanvasWithoutMerge({
  ...
  tool.removeNewFunctionalGroupAbbreviations(restruct);
